...
--CTE showing Customer Name associated with customerID
With CustName_CTE as (
	SELECT
		DISTINCT(customerID),
		customerNameCode
	FROM [Supply Chain Practice].[dbo].[trucking]
	WHERE customerID is not NULL AND customerNameCode is not NULL
	)
SELECT
	customerNameCode
	, customerID
